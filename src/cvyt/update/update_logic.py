...
class UpdateLogic():
    """Update logic for the update widnow/widget."""
    ...
    def process_archive(self, archive: str, to_folder: str = None):
        ...
        to_folder = optional path to the folder where the archive will be extracked
        """
        ...
        try:
            if zipfile.is_zipfile(archive):
                ...
                logger.info("Unknown archive format '%s'.", archive)
        except Exception as e:
            logger.error(
                "Processing of archive '%s' failed(%s).",
                archive,
                e)
    ...
    def initiate_update(self):
        ...
        try:
            # Let's try update each module from the archive
                ...
                if module.get('update', None):
                    ...
                    if root:
                        try:
                            ...
                            logger.warning(
                                """Could not finish updating item
                                      '%s' because %s.""", name, e
                            )
                            message = """Could not finish updating item
                                      '%s' because %s.""" % (name, e)
                            yield (False, message)
                    else:
                        logger.warning(
                            "No 'root' folder found for '%s'.", name)
                        message = """No 'root' folder found for '%s'.""" %\
                                  name
                        yield (False, message)
        except Exception as e:
            error_message = f"{e}"
    ...
    def get_available_modules(self):
        """Get an available module.

        Returns:
        tuple containing (name, module object)
        """
        return self.common_update.get_available_modules()

    def get_description_for_module(self, name: str) -> str | None:
        """Get the description for the selected module.
    ...
    def copy_archive_to_temp_folder(self, archive: str):
        """Copy the archive from its original location to a temp folder.

        Args:
        archive (str)= path to the archive to remove
        """
